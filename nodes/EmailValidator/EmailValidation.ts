...
async function checkMXRecord(domain: string): Promise<{ hasMX: boolean; mxRecords: string[]; error?: string }> {
	const timeout = 5000;
...
async function trySMTPPort(
	...
	useSTARTTLS: boolean,
	timeout: number
): Promise<{ success: boolean; acceptsEmail: boolean; error?: string }> {
	return new Promise((resolve) => {
		let socket: net.Socket | tls.TLSSocket | null = null;
		let tlsSocket: tls.TLSSocket | null = null;
		let responseBuffer = '';
		let smtpState: 'greeting' | 'ehlo' | 'starttls' | 'mailfrom' | 'rcptto' = 'greeting';
		...
		const succeed = (acceptsEmail: boolean, error?: string) => {
			cleanup();
		...
		const processSMTPResponse = (data: Buffer) => {
...
async function testSMTPConnection(domain: string, email: string, mxRecords: string[]): Promise<{
	canConnect: boolean;
	...
	let port25Error: string | undefined;
	...
	for (const portConfig of portsToTry) {
		...
		try {
			const result = await Promise.race([
				...
				new Promise<{ success: boolean; acceptsEmail: boolean; error?: string }>((_, reject) =>
					setTimeout(() => reject(new Error('Port timeout')), timeout)
				)
			]);
...
// Syntax validation
function validateEmailSyntax(email: string): { isValid: boolean; errors: string[]; providerSpecific?: any } {
	const errors: string[] = [];
	...
	const providerInfo = FREE_PROVIDERS[domain as keyof typeof FREE_PROVIDERS];
	if (providerInfo) {
...
function detectTypos(domain: string): { hasTypo: boolean; suggestions: string[] } {
...
async function checkDomainReputation(domain: string): Promise<{
	...
	if (FREE_PROVIDERS[domain as keyof typeof FREE_PROVIDERS] !== undefined && !majorProviders.includes(domain)) {
		score -= 5;
	...
	if (score >= 70) reputation = 'good';
	else if (score >= 40) reputation = 'neutral';
...
// Honeypot detection
function detectHoneypot(email: string): { isHoneypot: boolean; confidence: number; reasons: string[] } {
...
async function checkHaveIBeenPwned(email: string): Promise<{
	isPwned: boolean;
	breaches: number;
	lastBreach?: string;
	details?: any;
...
function detectFreeEmailProvider(domain: string): {
...
function fallbackEmailValidation(
	email: string,
	domain: string,
	localPart: string,
	mxCheck: { hasMX: boolean; mxRecords: string[] },
	reputationCheck: { reputation: 'good' | 'neutral' | 'poor'; score: number },
	isFreeProvider: boolean,
	...
	confidence: number;
	reasons: string[];
	estimatedStatus: 'valid' | 'invalid' | 'unknown';
} {
	const reasons: string[] = [];
	let confidence = 50;
	let isValid = true;
	...
	if (mxCheck.hasMX && mxCheck.mxRecords.length > 0) {
		...
		const hasTrustedMX = mxDomains.some(mx =>
			trustedMXProviders.some(trusted => mx.includes(trusted))
		);
	...
	let estimatedStatus: 'valid' | 'invalid' | 'unknown';
...
export async function validateEmail(email: string): Promise<any> {
	...
	const freeProviderInfo = FREE_PROVIDERS[domain as keyof typeof FREE_PROVIDERS];
	...
	let fallbackValidationResult: any = null;
	if (useFallbackValidation) {
		const majorProviders = ['gmail.com', 'yahoo.com', 'outlook.com', 'hotmail.com', 'icloud.com', 'protonmail.com'];
		const isMajorProvider = majorProviders.includes(domain);
		
		fallbackValidationResult = fallbackEmailValidation(
			email,
			domain,
			localPart,
			mxCheck,
	...
	if (useFallbackValidation && fallbackValidationResult) {
		...
		if (isMajorProvider) {
			...
			const smtpTimedOut = smtpCheck.error && (
				smtpCheck.error.toLowerCase().includes('timeout') ||
